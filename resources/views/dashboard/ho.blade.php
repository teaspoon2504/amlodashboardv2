@extends('layouts.app')
@section('title', 'Dashboard - Head Office')
@section('breadcrumb', 'Head Office')
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50/60">
                            ...
                            <th class="px-4 py-3 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider hidden md:table-cell">Team Lead</th>
                            <th class="px-4 py-3 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider hidden md:table-cell">Officer</th>
                            <th class="px-4 py-3 center text-[11px] font-bold text-slate-500 uppercase tracking-wider">Target</th>
                            <th class="px-4 py-3 text-center text-[11px] font-bold text-slate-500 uppercase tracking-wider">Done</th>
                            <th class="px-4 py-3 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider hidden sm:table-cell">Due</th>
                        </tr>
                    </thead>
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[11px] text-slate-400">{{ $task->regionalOffice->name ?? '-' }} · {{ $task->branchOffice->name ?? '-' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3.5 text-slate-600 text-[13px] hidden md:table-cell">{{ $task->teamLead->name ?? '-' }}</td>
                        ...
                        <tr class="hover:bg-slate-50/70 transition-colors">
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        <p class="font-semibold text-slate-800 text-[13px]">{{ $task->taskCategory->name }}</p>
                                        <p class="text-[11px] text-slate-400">{{ $task->regionalOffice->name ?? '-' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3.5 text-slate-600 text-[13px] hidden md:table-cell">{{ $task->officer->name ?? '-' }}</td>
